...
impl UserRepository {
    ...
    pub async fn get_by_id(&self, id: &Uuid) -> Result<Option<User>, sqlx::Error> {
        let user = sqlx::query_as::<_, User>(
            "SELECT id, username, elo, email, created_at, pass_hash FROM users WHERE id = $1",
        )
        .bind(id)
    ...
    pub async fn create(
        ...
        let user = sqlx::query_as::<_, User>("INSERT INTO users (username, email, pass_hash, created_at, elo) VALUES ($1, $2, $3, $4, $5) RETURNING id, username, email, pass_hash, created_at, elo")
            .bind(username)
            .bind(email)
            .bind(pass_hash)
            .bind(Utc::now())
    ...
    pub async fn exists_by_username(&self, username: &String) -> Result<bool, sqlx::Error> {
        let exists: bool =
